...
class InvitationRepository[F[_]: Bracket[?[_], Throwable]](xa: Transactor[F]) extends InvitationAlgebra[F]{
  ...
  def select: F[List[(Invitation[UserId], InvitationId, Instant)]] = invitation.all.to[List].transact(xa)
  ...
  def opened(invitationId: InvitationId): F[Unit] = invitation.updateOpenTime(invitationId).run.transact(xa).void
