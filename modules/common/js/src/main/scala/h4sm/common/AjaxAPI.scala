...
abstract class AjaxAPI(implicit ec: ExecutionContext) extends API[Future] {
  ...
  protected def mapHeaders(req: XMLHttpRequest): Map[String, String] = threadHeaders.foldLeft(Map[String, String]()) {
    case (m, name) if req.getResponseHeader(name) != null => m + (name -> req.getResponseHeader(name))
    case (m , _) => m
  }

  case class Resp(val a: XMLHttpRequest) extends Response {
    def as[T: Decoder]: Future[T] = parser.decode[T](a.responseText).leftWiden[Throwable].liftTo[Future]
  }
  ...
  def postH[A: Encoder](route: String, a: A, hs: Map[String, String]): Future[(Map[String, String], Resp)] =
