...
object arbitraries {
  implicit def orderItemArbitrary[A: Arbitrary]: Arbitrary[OrderItem0[A]] =
    Arbitrary {
      for {
  ...
  implicit def orderArbitrary[A: Arbitrary, B: Arbitrary]: Arbitrary[Order0[A, B]] =
    Arbitrary {
      for {
  ...
  implicit def itemArbitrary[A: Arbitrary]: Arbitrary[Item0[A]] =
    Arbitrary {
      for {
