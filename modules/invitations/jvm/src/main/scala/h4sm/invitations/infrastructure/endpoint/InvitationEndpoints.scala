...
extends Http4sDsl[F] {
  ...
  def addInvite: UserAuthService[F, T] = UserAuthService {
    case req @ POST -> Root asAuthed _ => for {
      inv <- req.request.as[InvitationRequest]
      _ <- InvitationAlgebra[F].insert(Invitation[UserId](req.authenticator.asBase.identity, inv.toName, inv.toEmail))
      res <- Ok()
    } yield res
  }

  def openInvite: HttpRoutes[F] = HttpRoutes.of {
    case _ @ GET -> Root / "open" / inviteCode / emailStr => 
      for {
  ...
  def signupInvite: HttpRoutes[F] = HttpRoutes.of {
