...
extends UserPermissionAlgebra[F] {
  ...
  def byId(id: UserPermissionId): OptionT[F, (UserPermission[PermissionId], UserPermissionId, Instant)] =
    OptionT(userPermissions.byId(id).option.transact(xa))

  def insert(a: UserPermission[PermissionId]): F[Unit] = userPermissions.insert(a).run.as(()).transact(xa)
